...
```bash
TRANSITION=$(node "C:/code/rosty/.codex/get-shit-done/bin/gsd-tools.cjs" phase complete "${current_phase}")
```
...
```bash
PROGRESS=$(node "C:/code/rosty/.codex/get-shit-done/bin/gsd-tools.cjs" progress bar --raw)
```
...
if [ -n "$GSD_WORKSTREAM" ]; then
  WS_LIST=$(node "C:/code/rosty/.codex/get-shit-done/bin/gsd-tools.cjs" workstream list --raw)
fi
```
...
```bash
node "C:/code/rosty/.codex/get-shit-done/bin/gsd-tools.cjs" config-set workflow._auto_chain_active false
```
