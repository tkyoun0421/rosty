...
```bash
INIT=$(node "C:/code/rosty/.codex/get-shit-done/bin/gsd-tools.cjs" init phase-op "{{GSD_ARGS}}")
...
```bash
RESEARCHER_MODEL=$(node "C:/code/rosty/.codex/get-shit-done/bin/gsd-tools.cjs" resolve-model gsd-phase-researcher --raw)
```

## 1. Validate Phase

```bash
PHASE_INFO=$(node "C:/code/rosty/.codex/get-shit-done/bin/gsd-tools.cjs" roadmap get-phase "${phase_number}")
```
