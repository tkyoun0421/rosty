...
- `admin`은 직원용 초대 링크를 발급, 재발급, 비활성화하고 현재 상태를 확인할 수 있다.
- 무효 또는 만료된 초대 링크로는 가입을 완료할 수 없다.
- `admin`은 홀 기본 시급, 초과근무 기준, 직원별 시급을 수정할 수 있다.
