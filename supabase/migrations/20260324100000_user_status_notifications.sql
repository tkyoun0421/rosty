-- Patch admin_manage_member so suspend/reactivate actions enqueue access-state
-- notifications for the affected user.

do $$
begin
  if to_regclass('public.notifications') is null then
    raise exception 'public.notifications must exist before applying user_status_notifications';
  end if;

  if to_regtype('public.notification_type') is null then
    raise exception 'public.notification_type must exist before applying user_status_notifications';
  end if;

  if to_regprocedure('public.admin_manage_member(uuid, text, public.user_role)') is null then
    raise exception 'public.admin_manage_member(uuid, text, public.user_role) must exist before applying user_status_notifications';
  end if;
end;
$$;
...
begin
  ...
  if v_action = 'suspend' then
    ...
    with updated_member as (
      ...
      select
        updated_member.id,
        'user_suspended'::public.notification_type,
        'Access suspended',
        'Your access was suspended. Contact an admin if you think this is a mistake.',
        '/suspended',
        updated_member.id
      from updated_member
    )
  ...
  if v_action = 'reactivate' then
    ...
    with updated_member as (
      ...
      select
        updated_member.id,
        'user_reactivated'::public.notification_type,
        'Access restored',
        'Your access was reactivated. You can enter Rosty again.',
        case
          when updated_member.role = 'employee' then '/employee-home'
          else '/manager-home'
        end,
...
comment on function public.admin_manage_member(uuid, text, public.user_role)
  is 'Applies the current admin member actions without requiring broad direct update access on profiles and enqueues access-state notifications for approve, suspend, and reactivate actions.';
