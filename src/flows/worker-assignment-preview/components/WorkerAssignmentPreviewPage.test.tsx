...
function createAttendanceStatus(overrides: Partial<Record<string, unknown>> = {}) {
  return {
    assignmentId: "assignment-1",
    scheduleId: "schedule-1",
...
function mockWorkerAssignmentPage(options?: {
  ...
  attendanceStatuses?: Array<Record<string, unknown>>;
}) {
  getCurrentUser.mockResolvedValue({ id: "worker-1", email: "worker@example.com", role: "worker" });
  listConfirmedWorkerAssignments.mockResolvedValue(options?.assignments ?? [createAssignment()]);
  listWorkerAttendanceStatuses.mockResolvedValue(
    options?.attendanceStatuses ?? [createAttendanceStatus()],
  );
}
...
describe("WorkerAssignmentPreviewPage", () => {
  ...
  it("renders confirmed shifts with pay totals and open attendance actions when pay is ready", async () => {
    ...
    expect(screen.getByRole("heading", { name: "Confirmed shifts" })).toBeInTheDocument();
  ...
  it("renders the confirmed-work empty state and recruiting CTA when there is no confirmed shift", async () => {
    getCurrentUser.mockResolvedValue({ id: "worker-1", email: "worker@example.com", role: "worker" });
    ...
    expect(screen.getByText("No confirmed shift yet")).toBeInTheDocument();
    expect(
      screen.getByText(
        "When an admin finalizes your role, it will appear here with pay and check-in details.",
      ),
    ).toBeInTheDocument();
    expect(screen.getByRole("link", { name: "Browse recruiting schedules" })).toHaveAttribute(
      "href",
      "/worker/schedules",
    );
  });

  it("shows the pay-unavailable branch while keeping attendance actions available", async () => {
    mockWorkerAssignmentPage({
      assignments: [
        createAssignment({
    ...
    expect(screen.getByText("Pay details are not ready yet")).toBeInTheDocument();
    expect(
      screen.getByText(
        "Your shift is confirmed, but your hourly rate is still being assigned. Contact an admin if this stays unresolved.",
      ),
    ).toBeInTheDocument();
    expect(screen.getByText("Expected pay not available yet")).toBeInTheDocument();
    expect(screen.getAllByText("Pending admin rate").length).toBeGreaterThan(0);
    expect(screen.getByRole("button", { name: "Check in now" })).toBeEnabled();
  });
